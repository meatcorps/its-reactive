    ...
    where TDigest : IDigest, new()
{
    ...
    private void Init(byte[] key, byte[] macKey, IBlockCipherPadding padding)
    {
        ...
        this._blockCipher = new CbcBlockCipher(new TBlockCipher());
        this._cipher = new PaddedBufferedBlockCipher(this._blockCipher, padding);
        // ReSharper disable once HeapView.PossibleBoxingAllocation
        this._mac = new HMac(new TDigest());
        this._mac.Init(new KeyParameter(macKey));
    }
    ...
    {
        byte[] input = this._encoding.GetBytes(plain);

        var iv = this.GenerateIV();

        var cipher = this.BouncyCastleCrypto(true, input, new ParametersWithIV(new KeyParameter(_key), iv));
        byte[] message = CombineArrays(iv, cipher);

        this._mac.Reset();
        this._mac.BlockUpdate(message, 0, message.Length);
        byte[] digest = new byte[this._mac.GetUnderlyingDigest().GetDigestSize()];
        this._mac.DoFinal(digest, 0);
    ...
    public byte[] DecryptBytes(byte[] bytes)
    {
        ...
        var digest = new byte[this._mac.GetUnderlyingDigest().GetDigestSize()];
        var message = new byte[bytes.Length - digest.Length];
        var iv = new byte[this._blockCipher.GetBlockSize()];
        ...
        byte[] result = this.BouncyCastleCrypto(false, cipher, new ParametersWithIV(new KeyParameter(_key), iv));
        return result;
    }

    public string Decrypt(byte[] bytes)
    {
        return this._encoding.GetString(DecryptBytes(bytes));
    }
    ...
    private bool IsValidHMac(byte[] digest, byte[] message)
    {
        this._mac.Reset();
        this._mac.BlockUpdate(message, 0, message.Length);
        byte[] computed = new byte[this._mac.GetUnderlyingDigest().GetDigestSize()];
        this._mac.DoFinal(computed, 0);
    ...
    private byte[] GenerateIV()
    {
        ...
        {
            // 1st block
            byte[] result = new byte[this._blockCipher.GetBlockSize()];
